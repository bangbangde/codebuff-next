  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-8 sm:py-10 lg:py-12">
      <div className="flex flex-col gap-4 border-b border-border pb-4 sm:flex-row sm:items-center sm:justify-between">
        <nav aria-label="文章视图" className="flex items-center gap-1">
          <Link
        ...
        <div className="flex items-center gap-2">
          <form action={createDraftAction}>
            <Button type="submit">
              ...
              <span className="sr-only sm:hidden">创建文章</span>
            </Button>
          </form>
        </div>
      </div>

      <section aria-labelledby="article-list-title" className="pt-6">
        <h2 className="sr-only" id="article-list-title">
          {activeTab === "drafts" ? "草稿箱文章列表" : "已发布文章列表"}
        </h2>
        ...
        {visibleArticles.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border bg-muted px-5 py-10 sm:px-8 sm:py-12">
            ...
            <h3 className="mt-5 text-base font-semibold">
              {activeTab === "drafts" ? "草稿箱为空" : "还没有已发布文章"}
            </h3>
            <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
              {activeTab === "drafts"
                ? "数据库中暂无未发布文章。创建第一篇草稿，开始撰写内容。"
                : "暂无已发布文章。在草稿箱中编辑并发布后，会出现在这里。"}
            </p>
            {activeTab === "drafts" ? (
